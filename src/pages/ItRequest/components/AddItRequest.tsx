...
export default function AddItRequest({ open, onClose, onSuccess }: AddItRequestProps) {
  ...
  const handleSubmit = methods.handleSubmit(async (data) => {
    try {
      ...
      if (data.attachment && !fileUrl) return;
  ...
  return (
    <React.Fragment>
      <Dialog open={open} onClose={onClose} slotProps={{ paper: { component: 'form' } }}>
        <DialogTitle>IT Request</DialogTitle>
        <FormProvider {...methods}>
          <DialogContent>
            <CommonForm<ITRequestFormValues> fields={itRequestFormFields} onSubmit={() => {}} />
          </DialogContent>
        </FormProvider>
